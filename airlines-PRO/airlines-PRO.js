...
function buyFlight() {
  ...
  if (!isNaN(userPrice)) {
    alert(
      "\nLos siguientes vuelos encajan en su precio de: " +
        userPrice +
        " euros."
    );
    alert("Id\tOrigen\t\t\t\t\tDestino\t\t\t\tPrecio");
    showFlights(flights, "filteredByPrice", userPrice);
    let idFlightToBuy = window.prompt(
      "Indique el id del vuelo que quiere comprar: \n(Localice el id de vuelo en el listado de vuelos disponibles)"
    );
...
function delFlight() {
  let idToDelete = parseInt(
    prompt("¿Cúal es el id de vuelo que desea eliminar?: ")
  );
  if (isNaN(idToDelete)) {
    window.alert("Proceso cancelado.");
  } else {
    ...
    switch (typeOfView) {
      ...
      case "filteredByPrice":
        if (flights[i].cost <= userPrice) {
          ...
          textMessage =
            flights[i].id +
            "\t" +
            formatTextForView(flights[i].from) +
            "\t==>\t\t" +
            formatTextForView(flights[i].to) +
            "\t\t" +
            flights[i].cost;
          alert(textMessage);
        }
      ...
      case "conEscala":
        ...
        if (tieneEscala) {
          textoEscala = "si ";
          let textMessage =
            formatTextForView(flights[i].from) + "\t==>\t\t" + flights[i].to;
          alert(textMessage);
        }
        break;
      case "lastFlights":
