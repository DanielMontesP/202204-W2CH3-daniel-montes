  ...
  if (isFirstRun) {
    ...
    switch (userName) {
      ...
      default:
        if (
          !window.confirm(
            "¿Es correcta la configuración predeterminada?\nLinea: " +
              numLineas +
              " bolas.\nBolas sorteadas: " +
              numBolas
          )
  ...
  if (isNewGame) {
    ...
    if (!userHasCancelled) {
      ...
      let userAnswer = window.prompt(
        "Actualmente hay " +
          usersCarton.length / numLineas +
          " jugadores activos.\n¿Quiere visualizar sus cartones? (y/n).",
        "n"
      );
      iniStart: {
        switch (userAnswer) {
        ...
        let answer = window.prompt(
          "Escriba AUTO si quiere activar el modo AUTO?\nAUTO = Girar bombo hasta cantar linea.",
          "auto"
        );
        if (iNeedDebugger) {
          debugger;
...
function cambiarSettings() {
  numLineas = window.prompt(
    "Actualmente son " +
      numLineas +
      " para una linea.\n Indique otro número si quiere cambiarlo",
    5
  );
...
function modficarPuntos(usuario, newPuntos) {
  for (let i = 0; i < ranking.length; i++) {
    if (ranking[i].usuario == usuario) {
      ...
      alert(
        "Puntuación actual en el ranking " + usuario + ": " + ranking[i].puntos
      );
    }
  }
}
...
function addPuntos(user, points) {
  ...
  alert("Puntos totales en el ranking: " + points);
}
...
function hayLinea() {
  ...
  for (let i = 0; i < usersCarton.length; i++) {
    ...
    if (usersCarton[i].premiado == true) {
      ...
      if (numAciertos == numLineas) {
        alert("\n¡¡¡¡Linea!!!!");
        alert("Ganador: " + usersCarton[i].usuario);
        alert("Línea conseguida en la tirada: " + numSorteos);
  ...
  alert("No hay lineas.");
  alert("Lista de bolas premiadas hasta el momento: ");
  mostrarCarton(bolasPremiadas);
  alert("Número de bolas sorteadas hasta el momento: " + bolasPremiadas.length);
}
...
function getUserData(user) {
  ...
  for (let i = 0; i < usersCarton.length; i++) {
    if (usersCarton[i].usuario == user) {
      userData.push({
        usuario: usersCarton[i].usuario,
        numero: usersCarton[i].numero,
        premiado: usersCarton[i].premiado,
      });
    }
  }
  return userData;
...
function actualizarCartones(newBola) {
  let posBegin = 0;
  while (
    usersCarton.includes(
      usersCarton.find((el) => el.numero === newBola),
      posBegin
    )
  ) {
    ...
    alert(
      usersCarton[posBegin].usuario + " tiene el " + newBola + " en su cartón."
    );
