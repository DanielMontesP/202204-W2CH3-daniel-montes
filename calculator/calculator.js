...
function askForValue() {
  ...
  if (value1 == 0) {
    textMens =
      "Vamos a realizar operaciones aritméticas. Introduzca un valor numérico: ";
  } else {
    textMens =
      "Si presiona la tecla Enter se calculará la raiz cuadrada de: " +
      value1 +
      ".\nSi introduce un número se calculará la suma, resta, multiplicación y división de: " +
      value1 +
  ...
  switch (userInput) {
    case "":
      if (value1 == 0) {
        window.alert(
          "Debe introducir un valor numérico, intentelo de nuevo..."
        );
        exeCalculator();
      } else {
        value2 = "";
  ...
  if (parseInt(numToProcess) > 0) {
    if (numToProcess.slice(2, numToProcess.length) == 0) {
      ...
      if (numToReturn.slice(2, numToReturn.length) == 0) {
        numToReturn = parseFloat(
          numInt + "." + numToReturn.slice(0, numToReturn.length - 1)
        );
      }
    } else {
  ...
  switch (typeOfCalc) {
    ...
    case "RaizCuadrada":
      ...
      textoMensaje =
        ...
        value1 +
        " es = " +
        formatResult(raizCuadrada.toFixed(3)) +
        ".\n\n¡Gracias por utilizar la calculadora!";
      break;
  }
